...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const staticRoutes: MetadataRoute.Sitemap = [
    ...
    { url: `${BASE_URL}/sports`, lastModified: new Date(), changeFrequency: 'daily', priority: 0.9 },
    { url: `${BASE_URL}/schedule`, lastModified: new Date(), changeFrequency: 'hourly', priority: 0.85 },
  ...
  const dedupedMatches = [...new Map([...live, ...popular].map((m) => [m.id, m])).values()]
